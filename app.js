...
async function loadStockChart(symbol, range) {
  ...
  const fromMap = { '1h':now-3600, '1d':now-86400, '7d':now-86400*7, '30d':now-86400*30, '365d':now-86400*365 };
  const resMap  = { '1h':'5', '1d':'30', '7d':'60', '30d':'D', '365d':'W' };
  const from    = fromMap[range] || fromMap['1d'];
  const res     = resMap[range]  || '30';
  ...
  try {
    ...
    if (!d || d.s !== 'ok' || !d.t || !d.t.length) throw new Error('no candle data');
